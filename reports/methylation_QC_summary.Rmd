...
Only one sample was excluded at this point, due to a deviation in the ratio between median methylated signal and median unmethylated signals. When repeating the QC without this sample, no other samples was removed.
...
`r nrow(probes)` were discarded in this point: `r sum(probes$issue == "Low bead number")` probes due to a low bead number and `r sum(probes$issue == "Detection p-value")` due to low call rate. 
...
In our dataset, we had a sample with a technical duplicate (G2CS12CO). We evaluated whether the methylation values of these two samples were more similar than the rest of methylation values. To do use, we used a centered correlation.
...
- Design. These variables are related with our experimental design: Status, CHD group and sample Batch.

None of the first 5 Principal components was associated with any of the technical variables, suggesting that normalization has removed technical noise. Of the phenotypic variables, gestational age was correlated with components 1 to 4, while sex was not correlated. Design variables had a strong influence in overall methylation patterns. The strongest variable was sample batch, which was correlated with components 1, 2 and 5. Then, we had case/control status (components 3 and 4) and pathological group (component 4). Finally, we observed that having a genetic cause was correlated with component 1. Full results of this exploration are available in meffil_v1_2020-02-21_normalization.html. 
