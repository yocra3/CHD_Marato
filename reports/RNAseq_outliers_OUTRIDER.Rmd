...
```{r}
vars <- ods.res$Pos
names(vars) <- ods.res$SampleID
vars <- vars[!is.na(vars)]
vars <- strsplit(vars, "/")
varsGR <- GRanges(unlist(vars))
varsGR$SampleID <- rep(names(vars), lengths(vars))
...
varRes <- lapply(seq_len(length(varsGR)), varOutliers, varRange = varsGR,
                 outrider = outres, geneRanges = GRgenes, window = 100e3)
varRes <- Reduce(rbind, varRes)
varRessig <- subset(varRes, pValue < 0.05) %>% unique() %>% arrange(pValue)
varRessig
```

Only four genes were nominally significant and close to a candidate genetic variant. These genes corresponded to two samples (G2CS6CO and G4CS8CO). Outlier genes for G2CS6CO were far from the candidate variants. For G4CS8CO, one outlier gene was far from the variant but another outlier gene matched the gene with a candidate variant:
