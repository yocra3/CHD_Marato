...
#' - segDups50: overlap with segmental duplications from UCSC. Segmental duplication contains >50% sample CNV.
#' - segDups90: overlap with segmental duplications from UCSC. Segmental duplication contains >90% sample CNV.
...
## Add overlaps with segmental duplications ####
segDupsGR <- makeGRangesFromDataFrame(segDups, start.field = "chromStart",
                                      end.field = "chromEnd")
seqlevelsStyle(segDupsGR) <- "NCBI"
names(segDupsGR) <- as.character(segDupsGR)

cnvGR$segDups50 <- lapply(seq_len(length(cnvGR)), function(i)
    getOverlapSampleRef(cnvGR[i], segDupsGR, overlap = 0.5))
cnvGR$segDups90 <- lapply(seq_len(length(cnvGR)), function(i)
    getOverlapSampleRef(cnvGR[i], segDupsGR[cnvGR$segDups50[[i]]], overlap = 0.9))
...
Description = c("any overlap with common calls from nstd186 (AF > 0.01)",
  ...
  "overlap with segmental duplications from UCSC. Segmental duplication contains >50% sample CNV",
